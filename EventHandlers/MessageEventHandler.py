...
async def runTelofikMessage(message):
    for embed in message.embeds:
        ...
        if 's+topall' in desc:
            ...
            await sendMessageAsTelofik(f'Dear {username}, full list: {url} {generateAntiSpam()}')
        ...
        if 's+check' in desc:
            ...
            try:
                weight = await getWeight(name)
                await sendMessageAsTelofik(f"{username}, {name}'s weight: {weight}")
            except:
                await sendMessageAsTelofik(f"{username}, Something went wrong!")
